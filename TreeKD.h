...
#define KD_MAX_POINTS 1000    //max KD tree points number  
#define SD 2					//current space dimension
#define RMAX 1000
...
#define KDTREE_MUKUL		KDTreeMukul<Xtype>
...
KDTEMPLATE
bool KDTreeMukul<Xtype>::add(Xtype* x)
{
...
KDTEMPLATE	
KDNODE* KDTreeMukul<Xtype>::find_nearest_brute(Xtype* x) 
{
	if(!Root)
		return NULL;
...
KDTEMPLATE
KDNODE* KDTREE_MUKUL::find_nearest(Xtype* x)
{
	if(!Root)
		return NULL ;
...
KDTEMPLATE
void KDTREE_MUKUL::check_subtree(KDNODE* node, Xtype* x)
{
...
KDTEMPLATE
void KDTREE_MUKUL::set_bounding_cube(KDNODE* node, Xtype* x)
{
	if(!node)
		return ;
	int d=0;
...
KDTEMPLATE
KDNODE* KDTREE_MUKUL::search_parent(KDNODE* parent, Xtype* x)
{
	for(int k=0; k<SD; k++)
	{
		x_min[k] = x_max[k] = 0;
